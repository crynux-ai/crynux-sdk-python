...
from crynux_sdk.config import get_default_tx_option
...
def read_abi(name: str):
    file = impresources.files("crynux_sdk.contracts.abi") / f"{name}.json"
        ...
        async def _wait_receipt(w3: AsyncWeb3):
            ...
            if not receipt["status"]:
                try:
                    ...
                    _logger.error(
                        f"static_call error when getting revert reason of tx {self.tx_hash.hex()}"
                    )
                    _logger.exception(e)
